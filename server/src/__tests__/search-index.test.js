...
describe('Search Index', () => {
  ...
  describe('BM25 search', () => {
    ...
    before(() => {
      ...
      insertChunk.run(1, 0, 'Machine learning is a branch of artificial intelligence focused on building systems that learn from data.', 1, 0, 100);
      insertChunk.run(1, 1, 'Deep learning uses neural networks with multiple layers to process complex patterns in data.', 1, 100, 200);
      insertChunk.run(1, 2, 'Natural language processing enables computers to understand and generate human language.', 2, 0, 100);
      insertChunk.run(1, 3, 'Computer vision allows machines to interpret visual information from the world around them.', 2, 100, 200);
      insertChunk.run(1, 4, 'Reinforcement learning trains agents through rewards and penalties in an environment.', 3, 0, 100);
    ...
    it('should rank results by relevance', () => {
      ...
      assert.ok(results.length > 0);
      assert.ok(results[0].content.includes('Deep learning') || results[0].content.includes('neural'));
    });

    it('should return page numbers', () => {
      const results = search(db, 1, 'natural language');
      assert.ok(results.length > 0);
      const nlpResult = results.find((r) => r.content.includes('Natural language'));
      assert.ok(nlpResult);
